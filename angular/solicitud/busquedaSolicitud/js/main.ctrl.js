app.controller('busquedaClienteController', function($scope, $notify, $http, $rootScope, $timeout, $sce, $location) {
    ...
    $scope.eliminaSolicitud = function(id_solicitud, id_solicitud_cliente) {
        ...
        var mail = {
            ...
            contenido: 'Solicitud ' + id_solicitud + ' cancelada por ' + $rootScope.loggedUser.nombre
        };
        ...
        $http.get('/api/usuarioId/' + 1).success(function(respu) {
            var mail = {
                ...
                para: respu[0].correo,
                asunto: 'Alerta Qualified',
                contenido: 'Solicitud ' + modalSol.id_solicitud + ' cancelada por ' + $rootScope.loggedUser.nombre
            };
            $http.post('/api/mail', mail).success(function(respuesta) {});
        });
    };
    ...
    $scope.confirmaRecepcion = function(modalSol) {
            ...
            var mail = {
                ...
                para: $rootScope.loggedUser.correo,
                asunto: 'Alerta Qualified',
                contenido: 'Solicitud ' + modalSol.id_solicitud_cliente + ' recibida por ' + $rootScope.loggedUser.nombre
            };
            $http.post('/api/mail', mail).success(function(respuesta) {});
            ...
            $http.get('/api/usuarioId/' + 1).success(function(respu) {
                var mail = {
                    ...
                    para: respu[0].correo,
                    asunto: 'Alerta Qualified',
                    contenido: 'Orden de Trabajo ' + modalSol.id_orden_trabajo + ' recibida por ' + $rootScope.loggedUser.nombre
                };
                $http.post('/api/mail', mail).success(function(respuesta) {});
            });
            $http.get('/api/usuarioId/' + modalSol.inspector).success(function(respu) {
...
app.controller('modalActualizacionHarina', function($scope, $http, $rootScope) {
