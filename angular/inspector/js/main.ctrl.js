app.controller('inspectorController', ['$scope', '$http' , '$rootScope', '$notify', '$timeout', 'Upload', '$sce',function ($scope, $http ,$rootScope , $notify, $timeout, Upload, $sce) {
  ...
  function cambiaEstado(solicitud, lotes){
    ...
    if (cont === lotes.length) {
      var pro = [{"id_proceso" : 8, "descripcion" : "Laboratorio", "color" : "2E64FE", "icono" : "flask", "visible" : true}];
      var nuevo = {id_solicitud: solicitud, proceso: pro};
  ...
  $scope.aprobarInspeccion = function(modalSol){
    ...
    if (validacion[0].sellos === undefined) {
      ...
      if (cont === validacion.length) {
        var nuevo = [{"id_proceso" : 8, "descripcion" : "Laboratorio", "color" : "2E64FE", "icono" : "flask", "visible" : true}];
        $http.put('/api/solicitud/' + modalSol.id_solicitud, {id_orden_trabajo: modalSol.id_orden_trabajo, estado: modalSol.estado, proceso: nuevo, inspector: modalSol.inspector, laboratorio: modalSol.laboratorio,fecha_planificacion: modalSol.fecha_planificacion}).success(function(response){
        });
        $http.put('/api/solicitudEncargado/' + modalSol.id_solicitud, {encargado: modalSol.laboratorio.nombre}).success(function(resultado){
        });

        var aviso = {usuario: $rootScope.loggedUser.id_usuario, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'ApruebaInspeccion', status: 'fa-search ', id_solicitud: modalSol.id_solicitud};
        $http.post('/api/avisos', aviso).success(function(resultado){
        })

        var aviso1 = {usuario: 1, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'ApruebaInspeccion', status: 'fa-search ', id_solicitud: modalSol.id_solicitud};
        $http.post('/api/avisos', aviso1).success(function(resultado){
        })

        var aviso2 = {usuario: modalSol.laboratorio, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'AsignaLaboratorio', status: 'fa-flask ', id_solicitud: modalSol.id_solicitud};
        $http.post('/api/avisos', aviso2).success(function(resultado){
        })

        $http.get('/api/usuarioId/' + 1).success(function(respu){
          var mail = {nombre: 'Qualified', para: respu[0].correo, asunto: 'Alerta Qualified', contenido: 'Inspeccion aprobada por ' + $rootScope.loggedUser.nombre + ' para la orden de trabajo: ' + modalSol.id_orden_trabajo};
          $http.post('/api/mail', mail).success(function(respuesta){});
        });
        $http.get('/api/usuarioId/' + laboratorio).success(function(respu){
          var mail = {nombre: 'Qualified', para: respu[0].correo, asunto: 'Alerta Qualified', contenido: 'Envio de muestras por ' + $rootScope.loggedUser.nombre + ' para la orden de trabajo: ' + modalSol.id_orden_trabajo};
          $http.post('/api/mail', mail).success(function(respuesta){});
        });
  ...
  $scope.modalMensaje = function (modalSol, msj) {
    ...
    if (modalSol.laboratorio !== undefined) {
      var aviso2 = {usuario: modalSol.laboratorio, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'mensajeNuevo', status: 'fa-envelope ', id_solicitud: modalSol.id_solicitud};
  ...
  $scope.desapruebaSoliciud = function(modalSol, observacion){
    var nuevo = [{"id_proceso" : 7, "descripcion" : "Desaprueba Inspeccion", "color" : "FF0000", "icono" : "exclamation-triangle", "visible" : true}];
    $http.put('/api/solicitud/' + modalSol.id_solicitud, {id_orden_trabajo: modalSol.id_orden_trabajo, estado: modalSol.estado, proceso: nuevo, inspector: '', laboratorio: '',fecha_planificacion: ''}).success(function(response){
      var id_usuario = $rootScope.loggedUser.id_usuario;
      var listaEstados = [];
      fechaActual = new Date();
      $http.get('/api/inspector/' + id_usuario).success(function(response){
    ...
    var observaciones = {id_solicitud: modalSol.id_solicitud, observacion: observacion};
    $http.post('/api/observacion', observaciones).success(function(respuesta){
    })

    var aviso = {usuario: $rootScope.loggedUser.id_usuario, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'DesapruebaInspeccion', status: 'fa-search ', id_solicitud: modalSol.id_solicitud};
    $http.post('/api/avisos', aviso).success(function(resultado){
    })

    var aviso1 = {usuario: 1, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'DesapruebaInspeccion', status: 'fa-search ', id_solicitud: modalSol.id_solicitud};
    $http.post('/api/avisos', aviso1).success(function(resultado){
    })

    $http.get('/api/usuarioId/' + 1).success(function(respu){
      var mail = {nombre: 'Qualified', para: respu[0].correo, asunto: 'Alerta Qualified', contenido: 'Inspeccion desaprobada por ' + $rootScope.loggedUser.nombre + ' para la orden de trabajo: ' + modalSol.id_orden_trabajo};
      $http.post('/api/mail', mail).success(function(respuesta){});
    });
  ...
  $scope.uploadPic = function(file, modalSol) {
    ...
    $http.get('/api/solicitudUnica/' + modalSol.id_solicitud).success(function(respuesta){
      ...
      var fecha = new Date();
      lista.push({nombre: file.name, fecha: fecha})
      $http.put('/api/solicitudEvidencia/' + modalSol.id_solicitud , {evidencia: lista}).success(function(resultado){
      })
    })
  ...
  $scope.desaprovarTransporte = function(modalSol, observacion){
    var nuevo = [{"id_proceso" : 13, "descripcion" : "Rechaza Transporte", "color" : "FF0000", "icono" : "truck", "visible" : true}];
    $http.put('/api/solicitud/' + modalSol.id_solicitud, {id_orden_trabajo: modalSol.id_orden_trabajo, estado: modalSol.estado, proceso: nuevo, inspector: modalSol.inspector, laboratorio: modalSol.laboratorio,fecha_planificacion: modalSol.fecha_planificacion}).success(function(response){
    });
    var observaciones = {id_solicitud: modalSol.id_solicitud, observacion: observacion};
    $http.post('/api/observacion', observaciones).success(function(respuesta){
    ...
    var aviso = {usuario: $rootScope.loggedUser.id_usuario, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'DesapruebaTransporte', status: 'fa-truck ', id_solicitud: modalSol.id_solicitud};
    $http.post('/api/avisos', aviso).success(function(resultado){
    })

    var aviso1 = {usuario: 1, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'DesapruebaTransporte', status: 'fa-truck ', id_solicitud: modalSol.id_solicitud};
    $http.post('/api/avisos', aviso1).success(function(resultado){
    })

    $http.get('/api/usuarioId/' + 1).success(function(respu){
      var mail = {nombre: 'Qualified', para: respu[0].correo, asunto: 'Alerta Qualified', contenido: 'Transporte desaprobado por ' + $rootScope.loggedUser.nombre + ' para la orden de trabajo: ' + modalSol.id_orden_trabajo};
      $http.post('/api/mail', mail).success(function(respuesta){});
    });
    var mail = {nombre: 'Qualified', para: modalSol.cliente.correo, asunto: 'Alerta Qualified', contenido: 'Carga del vehiculo desaprobada por ' + $rootScope.loggedUser.nombre + ' para la solicitud: ' + modalSol.id_solicitud_cliente};
    $http.post('/api/mail', mail).success(function(respuesta){});
  ...
  $scope.aprovarTransporte = function(modalSol){
    var nuevo = [{"id_proceso" : 12, "descripcion" : "Aprueba Transporte", "color" : "2E2EFE", "icono" : "truck", "visible" : true}];
    $http.put('/api/solicitud/' + modalSol.id_solicitud, {id_orden_trabajo: modalSol.id_orden_trabajo, estado: modalSol.estado, proceso: nuevo, inspector: modalSol.inspector, laboratorio: modalSol.laboratorio,fecha_planificacion: modalSol.fecha_planificacion}).success(function(response){
    });

    $http.put('/api/solicitudEncargado/' + modalSol.id_solicitud, {encargado: modalSol.cliente.nombre}).success(function(resultado){
    });

    var aviso = {usuario: $rootScope.loggedUser.id_usuario, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'ApruebaTransporte', status: 'fa-truck ', id_solicitud: modalSol.id_solicitud};
    $http.post('/api/avisos', aviso).success(function(resultado){
    })

    var aviso1 = {usuario: 1, name: 'Orden de Trabajo', id: modalSol.id_orden_trabajo, online: 'ApruebaTransporte', status: 'fa-truck ', id_solicitud: modalSol.id_solicitud};
    $http.post('/api/avisos', aviso1).success(function(resultado){
    })

    $http.get('/api/usuarioId/' + 1).success(function(respu){
      var mail = {nombre: 'Qualified', para: respu[0].correo, asunto: 'Alerta Qualified', contenido: 'Transporte aprobado por ' + $rootScope.loggedUser.nombre + ' para la orden de trabajo: ' + modalSol.id_orden_trabajo};
      $http.post('/api/mail', mail).success(function(respuesta){});
    });
    var mail = {nombre: 'Qualified', para: modalSol.cliente.correo, asunto: 'Alerta Qualified', contenido: 'Carga del vehiculo aprobada por ' + $rootScope.loggedUser.nombre + ' para la solicitud: ' + modalSol.id_solicitud_cliente};
    $http.post('/api/mail', mail).success(function(respuesta){});
  ...
  $scope.filtrarProceso = function(id_proceso){
    var lista = [];
    var inspector = {id_inspector: $rootScope.loggedUser.id_usuario};
    fechaActual = new Date();
    if (id_proceso === null) {
      traerSolicitudesFiltro();
    }else{
    ...
    if (asignados === 0) {
      ...
      if (lote.sellos === undefined) {
        if (lista.length === Number(lote.bultos)) {
          ...
          var lote = {id_solicitud: modalSol.id_solicitud, id_lote: lote.id_lote, sellos: nuevoSellos, asignados: asignados};
          $http.post('/api/loteSellos', lote).success(function(respu){
            $scope.listaLotesModal = respu[0].lote;
          })
          var nuevo = {usados: lista, numero: sellosInspector ,usuario: $rootScope.loggedUser.id_usuario};
  ...
  $scope.enviarModalImpresion = function(modalSol, item){
    ...
    $http.get('/api/usuarioId/' + modalSol.laboratorio).success(function(response){
      ...
      var nuevo = { 'asignados': item.asignados, 'sellos': item.sellos, 'id_lote': item.id_lote, 'materiaPrima': item.materiaPrima, 'trader': item.trader.nombreTrader, 'paisTrader': item.paisTrader,
        'lote': item.lote, 'bultos': item.bultos, 'cantidad': item.cantidad, 'contenedor': item.contenedor, 'bodega': item.bodega, 'estadoLab': item.estadoLab, 'muestreo': modalSol.tipoMuestreo.descripcion, 'bodega': item.bodega, 'traders': traders, 'laboratorio': laboratorio};
        lista.push(nuevo);
    ...
    var nuevo = {observacion: [{fecha: fecha, contenido: observacion, emisor: $rootScope.loggedUser.nombre, cargo: $rootScope.loggedUser.perfil[0].nombre_perfil, evidencias: archivos}], id_solicitud: solicitud.id_solicitud, id_lote: id_lote, id_solicitud_cliente: solicitud.id_solicitud_cliente, id_orden_trabajo: solicitud.id_solicitud_cliente};
    var observacion_lote = {observacion: nuevo};
    var id = {id_solicitud: solicitud.id_solicitud, id_lote: id_lote};
        ...
        $http.get('/api/solicitudUnica/' + solicitud.id_solicitud).success(function(sol){
          ...
          for (var i = 0; i < lotes.length; i++) {
            if (lotes[i].id_lote === id_lote) {
              lote = {'bodega': lotes[i].bodega, 'id_lote': lotes[i].id_lote, 'materiaPrima': lotes[i].materiaPrima, 'trader': lotes[i].trader, 'paisTrader': lotes[i].paisTrader,
                        'lote': lotes[i].lote, 'bultos': lotes[i].bultos, 'cantidad': lotes[i].cantidad, 'contenedor': lotes[i].contenedor, 'tipoMuestreo': lotes[i].tipoMuestreo, 'estadoLab': lotes[i].estadoLab ,'observaciones': true};
              lista.push(lote);
            }else{
              lista.push(lotes[i]);
            }
          }
          var post = {lotes: lista, id_solicitud: solicitud.id_solicitud};
