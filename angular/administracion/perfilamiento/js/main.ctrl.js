app.controller('administracionController', ['$scope', '$rootScope', '$notify', '$http',  function ($scope, $rootScope, $notify, $http) {
  ...
  //-------------------------------------------------------------------------------------------------------------------//
  $scope.crearUsuario = function(username, nombre, pass, correo, perfiles, trader, habilitaTrader){
    ...
    $http.post('/api/contador2', identificador).success(function(response){
      var usuario = {'id_usuario': response.seq, 'nombre': nombre, 'username': username, 'correo': correo, 'password': pass, 'trader': trader, 'alimento': alimento, 'animal': animal, 'agua': agua, 'perfil': perfiles};
