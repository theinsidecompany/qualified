...
exports.crearSolicitud = function(req, resp){
  ...
  var nuevaSolicitud = {'id_solicitud': id_solicitud, 'id_solicitud_cliente': id_solicitud_cliente, 'cliente': cliente, 'fecha_creacion': fecha_creacion, 'fecha_muestreo': fecha_muestreo, fecha_planificacion: fecha_planificacion, 'estado': estado, 'lote': lote, 'proceso': proceso, 'tipoMuestreo': tipoMuestreo};
...
exports.traerSolicitudesFiltroInspector = function(req, resp){
  var id = req.params.id_usuario;
  Solicitud.find({$and:[{'proceso.id_proceso': {$nin:[15]}} ,{inspector: id}]},function(err, solicitudes){
    if (solicitudes === null) {
      resp.send(null);
    }else{
      resp.send(solicitudes);
